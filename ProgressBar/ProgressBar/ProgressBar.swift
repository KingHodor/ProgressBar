...
class ProgressBar: UIView {
    ...
    func initializeSubviews() {
        ...
        let height = (24.0 - 8.0) / 2.0
        var index:CGFloat = 0
        
        for _ in 1...imageViewSize {
            let imageView = UIImageView(image: arrowGrayImage)
            imageView.frame = CGRect(x: Double(index), y: height, width: 6.0, height: 8.0)
            index = index + gapSize
    ...
    func startLoading(){
        ...
        for imageView in images{
            ...
            let imageViewYPosition:CGFloat = ((24.0 - 8.0) / 2.0 ) + 4//((myFrame?.height)! - 5) / 2
